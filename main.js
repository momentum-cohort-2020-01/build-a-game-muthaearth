...
class Warfare {
  ...
  update () {
    ...
    let noCollision = (c1) => {
return this.combatants.filter(function(c2) {return collision(c1, c2)}).length === 0}
...
foesBelow (foe) {
  ...
  return this.combatants.filter(function (c) {
    ...
    return c instanceof Foe &&
    Math.abs(foe.center.x - c.center.x) < c.size.x && 
    c.center.y > foe.center.y}).length > 0
  }
...
class Player {
  ...
  if(this.keyboarder.isDown(this.keyboarder.KEYS.S)) {
    ...
    let ammo = new Ammo({x: this.center.x, y: this.center.y - this.size.y - 10},
      {x:0, y:-7})
      //add ammo
      this.warfare.addCombatant(ammo)
